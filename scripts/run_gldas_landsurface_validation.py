#!/usr/bin/env python
"""Validate GEFSv12 land-surface forecasts against GLDAS Noah root-zone moisture.

This is a global model-product sensitivity check for the land-surface result.
It builds regional dry-fraction targets from GLDAS_NOAH025_M RootMoist_inst and
scores the existing GEFSv12 root-zone soil-moisture forecast probabilities
using the same validation/test logic as the ERA5-Land and NLDAS checks.

Because GLDAS-2.1 monthly starts in 2000, the default target normal and
calibration window is 2000-2016, with 2017-2019 held out for testing.
"""
...
RAW_DIR = PROJECT_ROOT / "data" / "raw" / "gldas_noah_monthly"
PROCESSED = PROJECT_ROOT / "data" / "processed"
OUT_DIR = PROJECT_ROOT / "outputs"
REPORT_DIR = PROJECT_ROOT / "results" / "report"
DEFAULT_VARIABLE = "RootMoist_inst"
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--gldas-dir", type=Path, default=RAW_DIR)
    ...
    parser.add_argument("--normal-start-year", type=int, default=2000)
    ...
    parser.add_argument("--out-prefix", default="landsurface_gldas_gefsv12_validation")
...
def raw_path(raw_dir: Path, month: pd.Timestamp) -> Path:
    return raw_dir / f"{month:%Y}" / f"GLDAS_NOAH025_M.A{month:%Y%m}.021.nc4"
...
def build_coverage_audit(args: Namespace) -> pd.DataFrame:
    ...
    for month in month_starts(args.start, args.end):
        path = raw_path(args.gldas_dir, month)
        rows.append(
            {
...
def choose_var(ds: xr.Dataset, requested: str) -> str:
    if requested in ds.data_vars:
        return requested
    fallbacks = [requested, "RootMoist_inst", "SoilMoi0_10cm_inst", "SoilMoi10_40cm_inst", "SoilMoi40_100cm_inst"]
    lower = {name.lower(): name for name in ds.data_vars}
    for name in fallbacks:
        if name.lower() in lower:
            return lower[name.lower()]
    raise ValueError(f"Could not find GLDAS soil-moisture variable. Requested={requested}; available={list(ds.data_vars)}")
...
def standardize_field(ds: xr.Dataset, variable: str, target_time: pd.Timestamp) -> xr.DataArray:
    ...
    if extra:
        raise ValueError(f"Unexpected GLDAS dimensions for {var}: {da.dims}")
    if "time" in da.dims and da.sizes["time"] == 1:
        da = da.squeeze("time", drop=True)
    if "time" in da.dims:
        raise ValueError(f"Unexpected multi-time monthly GLDAS field: {da.dims}")
...
def load_gldas_region_cube(region_slug: str, args: Namespace, audit: pd.DataFrame) -> xr.DataArray:
    fields = []
    ...
    for row in good_months.itertuples(index=False):
        ...
        try:
            ...
            ds.close()
    if not fields:
        raise FileNotFoundError("No valid GLDAS monthly NetCDF files are available.")
    cube = xr.concat(fields, dim="time").sortby("time")
    cube.name = "gldas_rootzone_sm"
    return cube
...
def observed_from_cube(cube: xr.DataArray, region_slug: str, args: Namespace) -> tuple[pd.DataFrame, xr.Dataset]:
    ...
    if int(normal_mask.sum()) < 12:
        raise ValueError("Not enough GLDAS normal-period months to define dry thresholds.")
        ...
        rows.append(
            {
                ...
                "observed_gldas_rzsm": float(field.mean(dim=["latitude", "longitude"], skipna=True).values),
            }
        )
    ...
    target_ds = xr.Dataset({"gldas_rootzone_sm": cube})
    target_ds.attrs.update(
        {
            "source": "GLDAS_NOAH025_M.2.1",
...
def fit_and_score(region: str, observed: pd.DataFrame, args: Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if merged.empty:
        raise ValueError(f"No GLDAS/GEFSv12 overlap for {region}.")
    ...
    if val.empty or test.empty:
        raise ValueError(f"Region {region} has empty validation or test split after GLDAS/GEFSv12 merge.")
...
def score_summary(
    ...
    for i, (model, col) in enumerate(candidates):
        ...
        rows.append(
            {
                "region": region,
                "target_product": "GLDAS_NOAH025_M",
...
def main() -> None:
    ...
    print(
        f"GLDAS coverage: {len(coverage) - missing}/{len(coverage)} monthly NetCDF files available "
    ...
    if missing and not args.allow_incomplete:
        write_outputs([], [], coverage, args)
        raise SystemExit(
            f"Missing or invalid GLDAS files: {missing}/{len(coverage)}. "
            "Run scripts/download_gldas_noah_monthly.py with Earthdata credentials first, "
    ...
    for region in regions:
        print(f"Processing GLDAS target and GEFSv12 validation for {region}", flush=True)
        cube = load_gldas_region_cube(region, args, coverage)
        observed, target_ds = observed_from_cube(cube, region, args)
        target_path = PROCESSED / f"gldas_noah_rootzone_monthly_{region}_{args.start[:4]}_{args.end[:4]}.nc"
