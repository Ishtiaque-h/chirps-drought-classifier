#!/usr/bin/env python
"""Download GLDAS Noah monthly root-zone soil-moisture files from NASA GES DISC.

GLDAS Noah monthly files are global and require Earthdata/GES DISC
authorization. This downloader mirrors the NLDAS downloader but targets
GLDAS_NOAH025_M.2.1, whose monthly GLDAS-2.1 record starts in 2000.
"""
...
USER_AGENT = "chirps-drought-classifier/gldas-target-sensitivity"
...
def parse_args() -> Namespace:
    ...
    parser.add_argument("--start", default="2000-01", help="First monthly file, YYYY-MM.")
...
def gldas_filename(month: pd.Timestamp) -> str:
    return f"GLDAS_NOAH025_M.A{month:%Y%m}.021.nc4"


def gldas_url(month: pd.Timestamp) -> str:
    return f"{BASE_URL}/{month:%Y}/{gldas_filename(month)}"


def destination(out_dir: Path, month: pd.Timestamp) -> Path:
    return out_dir / f"{month:%Y}" / gldas_filename(month)
...
def validate_download(path: Path) -> None:
    ...
    if "Access denied" in text or "Earthdata" in text or "Basic" in text:
        raise DownloadError(
            "GLDAS file download was denied. Configure Earthdata credentials and approve the NASA GES DISC app."
        )
...
def download_one(args: Namespace, session: requests.Session | None, month: pd.Timestamp, out_dir: Path) -> str:
    path = destination(out_dir, month)
    if path.exists() and looks_like_netcdf(path) and not args.overwrite:
        return "cached"
...
def main() -> None:
    ...
    print(f"GLDAS_NOAH025_M monthly files: {len(months)}")
    ...
    if args.dry_run:
        for month in months[:12]:
            print(f"{month:%Y-%m}: {gldas_url(month)} -> {destination(args.out_dir, month)}")
        if len(months) > 12:
    ...
    for i, month in enumerate(months, start=1):
        status = download_one(args, session, month, args.out_dir)
