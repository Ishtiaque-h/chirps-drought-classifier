...
def monthly_forecast_rows(
    ...
    if not path.exists():
        return
    df = pd.read_csv(path)
    if not {"y_true_dry_frac", "clim_prob_dry"}.issubset(df.columns):
        return
...
def add_memory_target(rows: list[dict[str, object]]) -> None:
    paths_by_name: dict[str, Path] = {}
    for base in [OUTPUTS, REPORT_DIR]:
        for path in sorted(base.glob("memory_target_*_summary.csv")):
    ...
    for path in paths_by_name.values():
        ...
        for record in df.to_dict(orient="records"):
            ...
            if variant == "lag_climate":
                model_prefix = "Memory target lag/climate"
            elif variant == "soil_memory":
                model_prefix = "Memory target soil-memory"
            elif variant == "external_operational":
                model_prefix = "Memory target external"
            else:
                model_prefix = "Memory target"
            display_model = (
                model
                if model == "target-consistent persistence"
                else f"{model_prefix} {model}"
            )
            note_bits = [
                f"variant={variant}",
                f"best_validation_calibration={record.get('best_validation_calibration', '')}",
            ]
            if pd.notna(record.get("spearman_obs_pred", np.nan)):
                note_bits.append(f"spearman={float(record['spearman_obs_pred']):.3f}")
            if pd.notna(record.get("amplitude_ratio", np.nan)):
                note_bits.append(f"amplitude_ratio={float(record['amplitude_ratio']):.3f}")
            if str(record.get("top_features", "")) not in {"", "nan"}:
                note_bits.append(f"top_features={record['top_features']}")
            calibration = str(record["calibration"])
            headline = (
                calibration == "selected"
                or (variant == "lag_climate" and model == "target-consistent persistence")
            )
            add_row(
                rows,
                category="central_valley_memory_target",
                scope=str(record.get("experiment", "memory_target")),
                region="California Central Valley",
                target=str(record["target"]),
                lead_months=int(record["lead_months"]),
                model=display_model,
                calibration=calibration,
                n_test_months=int(record["n_test_months"]),
                bs_reference=float(record["bs_reference"]),
                bs_model=float(record["bs_model"]),
                bss_vs_climatology=float(record["bss_vs_climatology"]),
                bss_ci_low=float(record["bss_ci_low"]),
                bss_ci_high=float(record["bss_ci_high"]),
                headline=headline,
...
def add_operational_benchmark(rows: list[dict[str, object]], n_bootstrap: int) -> None:
    ...
    for path in paths_by_name.values():
        ...
        if "cfsv2" in source_key:
            source_slug = (
                "ncei_cfsv2_rawamount"
                if "rawamount" in source_key
                else "ncei_cfsv2_anomaly"
            )
            model_prefix = "NCEI CFSv2 precipitation forecast"
            source_note = (
                "NOAA NCEI THREDDS CFSv2 individual-run precipitation "
                "accumulated over the target SPI window and mapped to monthly "
                "dry-fraction probability."
            )
        elif "prob" in source_key:
...
def add_landsurface_forecast_benchmark(rows: list[dict[str, object]], n_bootstrap: int) -> None:
    paths_by_name: dict[str, Path] = {}
    for base in [OUTPUTS, REPORT_DIR]:
        for path in sorted(base.glob("landsurface_cfsv2_*_monthly_scores.csv")):
            paths_by_name[path.name] = path
        for path in sorted(base.glob("landsurface_gefsv12_*hindcastcal*_monthly_scores.csv")):
    ...
    for path in paths_by_name.values():
        ...
        source_model = str(meta.get("source_model", pd.Series([""])).iloc[0])
        lead_months = (
            int(meta["benchmark_lead_months"].iloc[0])
            if "benchmark_lead_months" in meta.columns
            else 1
        )
        target = (
            str(meta["target_variable"].iloc[0])
            if "target_variable" in meta.columns
            else "ERA5-Land root-zone soil-moisture dry fraction"
        )
        region = (
            resolve_region(str(meta.get("region", pd.Series(["cvalley"])).iloc[0])).name
            if "region" in meta.columns
            else "California Central Valley"
        )
        scope = path.name.replace("_monthly_scores.csv", "")
        n_members = (
            float(meta["n_members"].iloc[0])
            if "n_members" in meta.columns and pd.notna(meta["n_members"].iloc[0])
            else np.nan
        )
        if "GEFSv12" in source_model:
            member_note = f"{int(n_members)}-member" if np.isfinite(n_members) else "ensemble"
            model_prefix = f"NOAA GEFSv12 RZSM reforecast {member_note}"
            pred_cols = {
                "raw_isotonic": "gefs_raw_isotonic_prob_dry",
                "anomaly_isotonic": "gefs_anom_isotonic_prob_dry",
                "selected": "gefs_selected_prob_dry",
            }
            note = (
                "NOAA GEFSv12 public reforecast soilw_bgrnd forecast state valid near "
                "target-month midpoint, mapped to ERA5-Land root-zone soil-moisture "
                "dry-fraction probability. The hindcast-calibrated run uses 2000-2016 "
                "for calibration and 2017-2019 for frozen testing. This is a "
                "forecast-informed land-surface benchmark, not a lag-only ML model."
            )
        else:
            cycle_note = "4-cycle mean" if np.isfinite(n_members) and n_members >= 3.5 else "single-cycle"
            model_prefix = (
                "NCEI CFSv2 soil-moisture forecast 4-cycle mean"
                if cycle_note == "4-cycle mean"
                else "NCEI CFSv2 soil-moisture forecast"
            )
            pred_cols = {
                "raw_isotonic": "cfsv2_raw_isotonic_prob_dry",
                "anomaly_isotonic": "cfsv2_anom_isotonic_prob_dry",
                "selected": "cfsv2_selected_prob_dry",
            }
            note = (
                "NOAA NCEI THREDDS CFSv2 monthly-mean flxf soilw forecast mapped "
                "to ERA5-Land root-zone soil-moisture dry-fraction probability. "
                "Observed dry thresholds and climatology use the train period; "
                "calibration is validation-only. This is a forecast-informed "
                f"land-surface benchmark, not a lag-only ML model; CFSv2 member aggregation={cycle_note}."
            )

        monthly_forecast_rows(
            path=path,
            pred_cols=pred_cols,
            category="forecast_informed_landsurface_benchmark",
            scope=scope,
            region=region,
            target=target,
            lead_months=lead_months,
        ...
        monthly_forecast_rows(
            path=path,
            pred_cols={
                "raw": "persistence_raw_prob_dry",
                "isotonic": "persistence_isotonic_prob_dry",
                "selected": "persistence_selected_prob_dry",
            },
            category="forecast_informed_landsurface_benchmark",
            scope=scope,
            region=region,
            target=target,
            lead_months=lead_months,
            model_prefix="ERA5-Land root-zone persistence",
            rows=rows,
            n_bootstrap=n_bootstrap,
            headline_methods={"raw", "selected"},
            note="Same observed root-zone dry-fraction target, using previous observed month as a land-memory reference.",
        )


def add_c3s_native_vsm_benchmark(rows: list[dict[str, object]]) -> None:
    path = REPORT_DIR / "paper" / "table39_c3s_ecmwf_native_vsm_benchmark.csv"
    if not path.exists():
        return
    df = pd.read_csv(path)
    for record in df.to_dict(orient="records"):
        ...
        if forecast.startswith("c3s_"):
            model = "C3S/ECMWF native VSM"
        else:
            model = "ERA5-Land root-zone persistence"
        bs_model = float(record["bs"])
        bss_value = float(record["bss"])
        bs_reference = bs_model / (1.0 - bss_value) if bss_value != 1.0 else np.nan
        add_row(
            rows,
            category="forecast_informed_landsurface_benchmark",
            scope=(
                f"landsurface_c3s_ecmwf_s51_vsm_{region_slug}_"
                f"{int(record['target_start_year'])}_{int(record['target_end_year'])}"
            ),
            ...
            bss_ci_low=float(record["bss_ci_low"]),
            bss_ci_high=float(record["bss_ci_high"]),
            headline=forecast in {"c3s_selected", "persistence_raw", "persistence_selected"},
            notes=(
                "C3S seasonal-original ECMWF system 51 native volumetric soil moisture; "
                "2019-2020 validation and complete 2021-2025 frozen test. "
                f"BSS versus selected persistence={float(record['bss_vs_persistence_selected']):+.3f}."
            ),
            source_file=str(path.relative_to(PROJECT_ROOT)),
        )
...
def main() -> None:
    ...
    add_memory_target(rows)
    add_operational_benchmark(rows, args.n_bootstrap)
    add_landsurface_forecast_benchmark(rows, args.n_bootstrap)
    add_c3s_native_vsm_benchmark(rows)
